...
class SubscriptionStatusRepository(private val subscriptionStatusDAO: SubscriptionStatusDao) {
    ...
    suspend fun upsert(subscriptionStatus: SubscriptionStatus): Long {
        return mutex.withLock {
            try {
                ...
                // Update timestamp
                subscriptionStatus.lastUpdatedTs = currentTimeMillis()
                ...
                println("IAB: " +  "$TAG Upsert completed with result: $result")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error during upsert: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun update(subscriptionStatus: SubscriptionStatus): Int {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Updating subscription: ${subscriptionStatus.productId}")
                val result = subscriptionStatusDAO.update(subscriptionStatus)
                println("IAB: " +  "$TAG Update result: $result")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error updating subscription: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun insert(subscriptionStatus: SubscriptionStatus): Long {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Inserting subscription: ${subscriptionStatus.productId}")
                val result = subscriptionStatusDAO.insert(subscriptionStatus)
                println("IAB: " +  "$TAG Insert result: $result")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error inserting subscription: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun getByPurchaseToken(token: String): SubscriptionStatus? {
        return try {
            ...
            println("IAB: " + "$TAG Error getting subscription by token: ${e.message}")
            null
        }
    }
    ...
    suspend fun getByProductId(productId: String): SubscriptionStatus? {
        return try {
            ...
            println("IAB: " + "$TAG Error getting subscription by product ID: ${e.message}")
            null
        }
    }
    ...
    suspend fun getByAccountId(accountId: String): List<SubscriptionStatus> {
        return try {
            ...
            println("IAB: " + "$TAG Error getting subscriptions by account ID: ${e.message}")
            emptyList()
        }
    }
    ...
    suspend fun updateStatus(id: Int, newStatus: Int, timestamp: Long): Int {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Updating subscription status: ID=$id, status=$newStatus")
                val result = subscriptionStatusDAO.updateStatus(id, newStatus, timestamp)
                println("IAB: " +  "$TAG Status update result: $result")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error updating status: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun updateBillingExpiry(id: Int, expiryTime: Long, timestamp: Long): Int {
        return mutex.withLock {
            try {
                ...
                println("IAB: " + "$TAG Error updating billing expiry: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun updateAccountExpiry(id: Int, expiryTime: Long, timestamp: Long): Int {
        return mutex.withLock {
            try {
                ...
                println("IAB: " + "$TAG Error updating account expiry: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun markExpiredSubscriptions(currentTime: Long): Int {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Marking expired subscriptions at time: $currentTime")
                val result = subscriptionStatusDAO.markExpiredSubscriptions(currentTime)
                println("IAB: " +  "$TAG Marked $result subscriptions as expired")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error marking expired subscriptions: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun deleteExpiredOlderThan(cutoffTime: Long): Int {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Deleting expired subscriptions older than: $cutoffTime")
                val result = subscriptionStatusDAO.deleteExpiredOlderThan(cutoffTime)
                println("IAB: " +  "$TAG Deleted $result expired subscriptions")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error deleting expired subscriptions: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun getInvalidSubscriptions(): List<SubscriptionStatus> {
        return try {
            ...
            println("IAB: " + "$TAG Error getting invalid subscriptions: ${e.message}")
            emptyList()
        }
    }
    ...
    suspend fun findDuplicateTokens(): List<TokenCount> {
        return try {
            ...
            println("IAB: " + "$TAG Error finding duplicate tokens: ${e.message}")
            emptyList()
        }
    }
    ...
    suspend fun removeDuplicateTokens(): Int {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Removing duplicate tokens")
                val result = subscriptionStatusDAO.removeDuplicateTokens()
                println("IAB: " +  "$TAG Removed $result duplicate subscriptions")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error removing duplicates: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun getStatusDistribution(): List<StatusCount> {
        return try {
            ...
            println("IAB: " + "$TAG Error getting status distribution: ${e.message}")
            emptyList()
        }
    }
    ...
    suspend fun getSubscriptionsExpiringInRange(
        ...
        return try {
            ...
            println("IAB: " + "$TAG Error getting subscriptions expiring in range: ${e.message}")
            emptyList()
        }
    }
    ...
    suspend fun delete(subscriptionStatus: SubscriptionStatus): Int {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Deleting subscription: ${subscriptionStatus.productId}")
                val result = subscriptionStatusDAO.delete(subscriptionStatus)
                println("IAB: " +  "$TAG Delete result: $result")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error deleting subscription: ${e.message}")
                throw e
            }
        }
    }
    ...
    suspend fun deleteAll(): Int {
        return mutex.withLock {
            try {
                println("IAB: " +  "$TAG Deleting all subscriptions")
                val result = subscriptionStatusDAO.deleteAllSubscriptions()
                println("IAB: " +  "$TAG Deleted all subscriptions: $result")
                result
            } catch (e: Exception) {
                println("IAB: " + "$TAG Error deleting all subscriptions: ${e.message}")
                throw e
            }
        }
    }
    ...
    private fun validateSubscriptionStatus(subscriptionStatus: SubscriptionStatus) {
        ...
        println("IAB: " +  "$TAG Subscription validation passed for: ${subscriptionStatus.productId}")
    }
