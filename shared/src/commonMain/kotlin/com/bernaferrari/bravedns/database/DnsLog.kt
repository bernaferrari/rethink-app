...
import com.bernaferrari.bravedns.platform.formatLogTime
...
class DnsLog {
    ...
    fun unansweredQuery(): Boolean {
        return this.status != "COMPLETE" || this.response == Constants.NXDOMAIN
    }

    fun isAnonymized(): Boolean = this.relayIP.isNotEmpty() || this.proxyId.isNotEmpty()

    fun isLocallyAnswered(): Boolean = this.serverIP.isEmpty()

    fun hasBlocklists(): Boolean = this.blockLists.isNotEmpty()

    fun getBlocklists(): List<String> = this.blockLists.split(",")
}
