...
class AppInfoRepository(private val appInfoDAO: AppInfoDAO) {
    ...
    suspend fun insert(appInfo: AppInfo): Long {
        appInfo.modifiedTs = currentTimeMillis()
    ...
    suspend fun updateUid(oldUid: Int, newUid: Int, pkg: String): Int {
        ...
        val modifiedTs = currentTimeMillis()
    ...
    suspend fun updateFirewallStatusByUid(uid: Int, firewallStatus: Int, connectionStatus: Int) {
        appInfoDAO.updateFirewallStatusByUid(uid, firewallStatus, connectionStatus, currentTimeMillis())
    }

    suspend fun updateTempAllowByUid(uid: Int, enabled: Boolean, expiryTime: Long) {
        appInfoDAO.updateTempAllowByUid(uid, enabled, expiryTime, currentTimeMillis())
    }
    ...
    suspend fun getAllTempAllowedAppsPaged(now: Long): androidx.paging.PagingSource<Int, AppInfo> {
        return appInfoDAO.getTempAllowedAppsPaged(now)
    }

    suspend fun clearTempAllowByUid(uid: Int) {
        appInfoDAO.clearTempAllowByUid(uid, currentTimeMillis())
    }

    suspend fun clearTempAllowByUidIfExpiry(uid: Int, expectedExpiry: Long): Int {
        return appInfoDAO.clearTempAllowByUidIfExpiry(uid, expectedExpiry, currentTimeMillis())
    }

    suspend fun cpUpdate(appInfo: AppInfo): Int {
        appInfo.modifiedTs = currentTimeMillis()
        return appInfoDAO.update(appInfo)
    }

    suspend fun cpUpdate(appInfo: AppInfo, clause: String): Int {
    ...
    suspend fun cpInsert(appInfo: AppInfo): Long {
        appInfo.modifiedTs = currentTimeMillis()
        return appInfoDAO.insert(appInfo)
    }

    suspend fun getAllAppDetailsForProvider(): List<AppInfo> = appInfoDAO.getAllAppDetailsCursor()

    suspend fun cpDelete(uid: Int): Int {
    ...
    suspend fun updateProxyExcluded(uid: Int, isProxyExcluded: Boolean) {
        appInfoDAO.updateProxyExcluded(uid, isProxyExcluded, currentTimeMillis())
    }
    ...
    /**
     * Blocking variant for use from non-coroutine contexts (eg: ExpiringCache removal listeners).
     */
    suspend fun clearTempAllowByUidBlocking(uid: Int) {
        appInfoDAO.clearTempAllowByUid(uid, currentTimeMillis())
    }
     ...
     * Blocking variant for use from non-coroutine contexts (eg: ExpiringCache removal listeners).
     */
    suspend fun clearTempAllowByUidIfExpiryBlocking(uid: Int, expectedExpiry: Long): Int {
        return appInfoDAO.clearTempAllowByUidIfExpiry(uid, expectedExpiry, currentTimeMillis())
    }

    suspend fun getNearestTempAllowExpiryBlocking(now: Long): Long? {
    ...
    suspend fun clearAllExpiredTempAllowsBlocking(now: Long): Int {
