...
fun WgAddPeerDialog(
    ...
    WgDialog(onDismissRequest = onDismiss) {
        WgDialogColumn(
            ...
            var publicKey by remember { mutableStateOf(wgPeer?.getPublicKey()?.base64().orEmpty()) }
            var presharedKey by remember {
                mutableStateOf(
                    wgPeer?.getPreSharedKey()?.base64().orEmpty()
                )
            }
            ...
            var endpoint by remember {
                mutableStateOf(
                    wgPeer?.getEndpoint()?.toString().orEmpty()
                )
            }
            var keepAlive by remember {
                mutableStateOf(
                    wgPeer?.persistentKeepalive?.toString().orEmpty()
                )
            }
            var keepAliveHint by remember {
                mutableStateOf(
                    wgPeer?.persistentKeepalive?.let { getDurationInHumanReadableFormat(context, it) }.orEmpty()
                )
            }
...
private suspend fun savePeer(
    ...
    try {
        ...
        Logger.e(Logger.LOG_TAG_UI, "Error while adding peer", e as? Exception)
