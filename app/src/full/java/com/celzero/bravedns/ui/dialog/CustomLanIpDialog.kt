...
fun CustomLanIpSheet(
    ...
    fun saveAutoMode() {
        try {
            ...
            Logger.e(LOG_TAG_UI, "err saving custom lan ip (auto): ${e.message}")
    ...
    fun saveManualMode() {
        try {
            ...
            Logger.e(LOG_TAG_UI, "err saving custom lan ip (manual): ${e.message}")
            errorMessage = context.getString(R.string.custom_lan_ip_save_error)
        }
    }
...
private fun validateIpv4WithPrefix(ip: String, prefixText: String): Boolean {
    ...
    return try {
        ...
        if (prefix !in 0..32) {
            Logger.w(LOG_TAG_UI, "IPv4 validation failed: prefix out of range: $prefixText")
            return false
        }
        true
    } catch (e: Exception) {
        Logger.w(LOG_TAG_UI, "IPv4 validation failed: ${e.message}")
        false
    }
}
...
private fun validateIpv6WithPrefix(ip: String, prefixText: String): Boolean {
    ...
    return try {
        ...
        if (prefix !in 0..128) {
            Logger.w(LOG_TAG_UI, "IPv6 validation failed: prefix out of range: $prefixText")
            return false
        }
        true
    } catch (e: Exception) {
        Logger.w(LOG_TAG_UI, "IPv6 validation failed: ${e.message}")
        false
    }
}
