...
import com.bernaferrari.bravedns.network.TcpProxyApi
import com.bernaferrari.bravedns.data.AppConfig
import com.bernaferrari.bravedns.database.TcpProxyEndpoint
import com.bernaferrari.bravedns.database.TcpProxyRepository
import com.bernaferrari.bravedns.scheduler.PaymentWorker
import com.bernaferrari.bravedns.util.Utilities.togs
...
object TcpProxyHelper : KoinComponent {
    ...
    private suspend fun publicKeyUsable(retryCount: Int = 0): Boolean {
        ...
        try {
            val response =
                TcpProxyApi.getPublicKey(
                    persistentState.routeRethinkInRethink,
                    persistentState.appVersion.toString(),
                )
            Logger.d(
                LOG_TAG_PROXY,
                "new tcp config: ${response.message}, ${response.requestUrl}",
            )
            ...
            if (response?.isSuccessful == true) {
                ...
                works = JsonHelper.getString(jsonObject, JSON_STATUS) == STATUS_OK
                val minVersionCode = JsonHelper.getString(jsonObject, JSON_MIN_VERSION_CODE)
                publicKey = JsonHelper.getString(jsonObject, JSON_PUB_KEY) ?: ""
                publicKeyJWK =
                    JsonHelper.encodeObject(JsonHelper.parseObject(publicKey))
                        .toByteArray(Charsets.UTF_8)
                Logger.i(
                    LOG_TAG_PROXY,
                    "tcp response for ${response.requestUrl}, works? $works, minVersionCode: $minVersionCode, publicKey: $publicKey"
                )
                return works
            } else {
                Logger.w(
