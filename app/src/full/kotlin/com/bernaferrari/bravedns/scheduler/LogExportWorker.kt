...
import com.bernaferrari.bravedns.database.ConsoleLogDAO
...
class LogExportWorker(context: Context, workerParams: WorkerParameters) :
    ...
    private fun exportLogsToCsvStream(filePath: String): Boolean {
        try {
            val query = RoomRawQuery(QUERY)
            val logs = kotlinx.coroutines.runBlocking { consoleLogDao.getLogsCursor(query) }
            ...
            for (log in logs) {
                stringBuilder.append("${log.timestamp},${log.message}\n")
            }
