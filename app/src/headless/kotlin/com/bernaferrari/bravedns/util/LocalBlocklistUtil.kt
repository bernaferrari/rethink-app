...
import com.bernaferrari.bravedns.util.Constants.Companion.ONDEVICE_BLOCKLISTS_IN_APP
import com.bernaferrari.bravedns.util.Constants.Companion.ONDEVICE_BLOCKLIST_FILE_BASIC_CONFIG
import java.io.File
...
class LocalBlocklistUtil(val context: Context, val persistentState: PersistentState) {
    ...
    fun init() {
        val jsonContent =
            context.assets
                .open(ONDEVICE_BLOCKLIST_FILE_BASIC_CONFIG.removePrefix(File.separator))
                .bufferedReader()
                .use { it.readText() }
        val jsonObject = JsonHelper.parseObject(jsonContent)

        val localBlocklistTimestamp =
            JsonHelper.getString(jsonObject, "timestamp")?.split("/")?.last()?.toLong() ?: return
