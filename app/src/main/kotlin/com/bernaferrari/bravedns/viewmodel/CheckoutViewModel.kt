    ...
    private fun handleKeys() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Logger.d(LOG_TAG_UI, "Public Key: $key")
                ...
                Logger.d(LOG_TAG_UI, "Blind: $keyState")
                val path =
                    File(
                ...
                Logger.d(LOG_TAG_UI, "Content: $content")
            } catch (e: Exception) {
                Logger.e(LOG_TAG_UI, "err in handleKeys: ${e.message}", Logger.throwableToException(e))
            }
        }
    }

    val paymentWorkInfo: Flow<List<WorkInfo>> =
        WorkManager.getInstance(app).workInfosByTagFlow(TcpProxyHelper.PAYMENT_WORKER_TAG)
