...
import com.bernaferrari.bravedns.service.VpnBuilderPolicy.Companion.getNetworkBehaviourDuration
import com.bernaferrari.bravedns.service.WireguardManager.NOTIF_CHANNEL_ID_WIREGUARD_ALERTS
import com.bernaferrari.bravedns.ui.HomeScreenActivity
import com.bernaferrari.bravedns.util.ConnectivityCheckHelper
import com.bernaferrari.bravedns.util.Constants.Companion.NOTIF_WG_PERMISSION_NAME
import com.bernaferrari.bravedns.util.Constants.Companion.NOTIF_WG_PERMISSION_VALUE
import com.bernaferrari.bravedns.util.InternetProtocol
import com.bernaferrari.bravedns.util.SsidPermissionManager
...
class ConnectionMonitor(private val context: Context, private val networkListener: NetworkListener, private val serializer: CoroutineDispatcher, private val scope: CoroutineScope) : KoinComponent, DiagnosticsManager.DiagnosticsListener {
    ...
    private class NetworkRequestHandler(
        ...
        // ref - https://developer.android.com/reference/kotlin/kotlin/util/LinkedHashSet
        ...
        private suspend fun determineVpnProtos(nws: Set<NetworkAndSsid?>): Pair<Boolean, Boolean>? {
            ...
            if (!has4 && !has6) {
                lp?.routes?.forEach rloop@{
                    // ref: androidxref.com/9.0.0_r3/xref/frameworks/base/core/kotlin/android/net/RouteInfo.java#328
        ...
        private suspend fun repopulateTrackedNetworks(
            ...
            nwProps.forEach outer@{ prop ->
                ...
                if (isCaptivePortal || relyOnValidation) {
                    ...
                    lp.routes.forEach rloop@{
                        // ref:
                        // androidxref.com/9.0.0_r3/xref/frameworks/base/core/kotlin/android/net/RouteInfo.java#328
                        hasDefaultRoute4 =
                            hasDefaultRoute4 ||
                                    (it.isDefaultRoute && it.destination.address is Inet4Address)
                        hasDefaultRoute6 =
