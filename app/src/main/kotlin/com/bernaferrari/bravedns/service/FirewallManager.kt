...
import com.bernaferrari.bravedns.service.FirewallManager.GlobalVariable.appInfos
import com.bernaferrari.bravedns.service.FirewallManager.GlobalVariable.appInfosFlow
import com.bernaferrari.bravedns.service.FirewallManager.GlobalVariable.foregroundUids
import com.bernaferrari.bravedns.util.AndroidUidConfig
import com.bernaferrari.bravedns.util.Constants.Companion.RETHINK_PACKAGE
import com.bernaferrari.bravedns.util.OrbotHelper
import com.bernaferrari.bravedns.util.ExpiringCache
import com.bernaferrari.bravedns.util.UidMultimap
...
object FirewallManager : KoinComponent {
    ...
    // androidxref.com/9.0.0_r3/xref/frameworks/base/core/kotlin/android/os/UserHandle.java
    private const val PER_USER_RANGE = 100000

    // lo part is the uid within the user and hi part is the userId
    // androidxref.com/9.0.0_r3/xref/frameworks/base/core/kotlin/android/os/UserHandle.java#224
    ...
    // androidxref.com/9.0.0_r3/xref/frameworks/base/core/kotlin/android/os/UserHandle.java#183
    ...
    object GlobalVariable {
        ...
        var appInfos: UidMultimap<AppInfo> = UidMultimap()

        // TODO: protect access to the foregroundUids (read/write)
        @Volatile var foregroundUids: HashSet<Int> = HashSet()

        var appInfosFlow: MutableStateFlow<Collection<AppInfo>> = MutableStateFlow(emptyList())
     ...
     * ExpiringCache uses a fixed write TTL; each entry stores the real expiry time in its value.
     */
    private val tempAllowCache: ExpiringCache<Int, Long> =
        ExpiringCache(
            maxSize = 10_000,
            expireAfterWriteMs = TimeUnit.MINUTES.toMillis(60),
            onRemoval = { uid, expiry, cause ->
                if (cause == ExpiringCache.RemovalCause.EXPIRED) {
    ...
    suspend fun tombstoneApp(uid: Int, packageName: String?, ts: Long = System.currentTimeMillis()) {
        ...
        mutex.withLock {
            val ai = appInfos.get(uid).firstOrNull { it.packageName == packageName }
            if (ai != null) {
                appInfos.remove(uid, ai)
                ai.uid = newUid
                ai.tombstoneTs = ts
                ai.modifiedTs = ts
    ...
    fun appListFlow(): StateFlow<Collection<AppInfo>> {
        return appInfosFlow
    }
    ...
    suspend fun updateUidAndResetTombstone(oldUid: Int, newUid: Int, pkg: String) {
        ...
        val now = System.currentTimeMillis()
        mutex.withLock {
            val ai = appInfos.get(oldUid).firstOrNull { it.packageName == pkg }
            if (ai != null) {
    ...
    private suspend fun getAppInfos(): Collection<AppInfo> {
        val observerSnapshot = appInfosFlow.value.toList()
    ...
    fun stats(): String {
        ...
        val snapshot = appInfosFlow.value.toList()
        snapshot.forEach {
    ...
    private suspend fun publishAppList() {
        // Snapshot under lock to avoid iterating the multimap while it is being mutated.
        val snapshot =
            mutex.withLock {
                if (appInfos.isEmpty()) {
                    emptyList()
                    ...
                    runCatching { appInfos.values().toList() }
                        .getOrElse { appInfosFlow.value.toList() }
                }
            }
        appInfosFlow.value = snapshot
    }
