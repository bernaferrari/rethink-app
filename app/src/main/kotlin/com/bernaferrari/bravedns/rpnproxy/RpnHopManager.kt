...
import com.bernaferrari.bravedns.database.WgHopMap
import com.bernaferrari.bravedns.database.WgHopMapRepository
...
import com.bernaferrari.bravedns.service.WireguardManager.INVALID_CONF_ID
...
object RpnHopManager: KoinComponent {
    ...
    suspend fun removeHop(srcId: Int, hopId: Int): Pair<Boolean, String> {
        val src = ID_RPN + srcId
        val hop = ID_RPN + hopId
    ...
    suspend fun hop(src: Int, hop: Int):  Pair<Boolean, String> {
        val srcId = ID_RPN + src
        val hopId = ID_RPN + hop
    ...
    suspend fun getHopableRpns(src: Int): List<Config> {
        val srcId = ID_RPN + src
        ...
        // remove the rpn which are part of other hops
    ...
    suspend fun handleRpnDelete(id: Int) {
        val idStr = ID_RPN + id
        // if the src is deleted, then remove all the maps which are using this src
        // this happens when the config is deleted during the update process
    ...
    fun isRpnEitherHopOrSrc(id: Int): Boolean {
        val idStr = ID_RPN + id
        val srcMap = getMapBySrc(idStr)
        val hopMap = getMapByHop(idStr)
